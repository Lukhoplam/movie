...
'''This will return a Language object containing all components and data needed 
to process text.'''
...
#Initialize a list to store movie list split into movie title 
...
#initialize list to store similarity values
    similarity_list = []
...
# check similarity between the movie description with the recently watched movie description
        similarity_list.append(nlp(list[i][1]).similarity(model_sentence))
...
# return movie title similar to watched movie
    return list[max_similarity_index][0]
